...
class EditTaskScreenState extends State<EditTaskScreen> {
  ...
  String? _formattedDate; // To hold formatted date string
  ...
  void initState() {
    ...
    _selectedCategory = widget.task.category; // Initialize category

    if (_dueDate != null) {
      _formattedDate = DateFormat('dd-MM-yyyy').format(_dueDate!);
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (pickedDate != null) {
      setState(() {
        _dueDate = pickedDate;
        _formattedDate = DateFormat('dd-MM-yyyy').format(pickedDate); // Format the date
        _dateController.text = _formattedDate!;
      });
    }
  }
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate() && _dueDate != null) {
                    final updatedTask = Task(
                      id: widget.task.id, // Assuming an ID field exists
                      name: _nameController.text,
                      description: _descriptionController.text,
                      dueDate: _dueDate!,
                      isComplete: widget.task.isComplete, // Preserve existing completion status
